...
const Runner = function (config) {
  ...
  switch (reduceType) {
    ...
    case types.pattern:
      ...
      let matchObj = patternMatchLines({ lines: sents, method: config.method, matchPattern: config.matchPattern })
      // TODO: if output failure AND method = patternStrats and  matchPattern
      // re-run up to 4 more times - if matchPattern and failure, retry wit no matchPattern
      if (matchObj.sentences.length === 0 && config.matchPattern) {
        // _sometimes_ this works
        // we should try MORE TIMES
        // but not forever, to forestall infinite loops
        matchObj = patternMatchLines({ lines: sents, method: config.method, matchPattern: null })
      }
