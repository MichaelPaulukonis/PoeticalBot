  ...
  let onePoem = function() {
    try {
      let Corpora = require('./corpora.js'),
          ...
          strategy,
          texts,
          // TODO: if drone specified and text specified, provide sentences
          // so we will have to check things.... later than here
          // cannot get a redefinition to work unless instantiated here. UGH
          drone = () => new Drone({util: util,
                                   sentences: util.pick(corpora.filter('sentence')).sentences()}).generate(),
          // drone = new Drone({util: util, sentences: []}),
          strategies = [ queneaubuckets,
                         jgnoetry,
                         drone];


      // TODO: if a file has been provided, use it
      // if the strategy is drone, then sentencify it
      // we should honor corpora restrictions for DRONE, as well...
      if (config.file) {
      ...
      // TODO: instantiate drone only after texts selected
      // ugh. exccept I can't get it to work if not instantiated above, as well. AAARGH.
      if (strategy == drone) {
        var sentences;
      ...
      if (!poem.title) {
        ...
        } while (poem.title.split(' ').length === 1 ); // hrm. this excludes '[UNTITLED]' doesn't it...
      }
