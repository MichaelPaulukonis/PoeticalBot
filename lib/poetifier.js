...
var Poetifier = function (options) {
  ...
  let fonetik = require(`../filter/fonetik`)
  ...
  let transformer = function (poem) {
    ...
    // remove many letters
    // disembowell (isn't that part of mis-speller?)
    // disemconsonant (isn't that part of mis-speller?)
    ...
    let strategy = util.pick(stragegies)
  ...
  let transformRhymer = function (poem) { // eslint-disable-line no-unused-vars
  ...
  let letterExistsFilterTransform = function (poem, letter) {
    ...
    let coreFilter = function (poem) {
      ...
      if (!poem.lines) { util.logger(`NO LINES PROPERTY FOUND`, JSON.stringify(poem, null, 2)) }
      poemCopy.lines = poemCopy.lines.map((line) =>
        line.split(` `) // NAIVE SPLITTING
