...
var Poetifier = function(options) {
  ...
  let jgnoetry = require('./jgnoetryRunner.js'),
      // TODO: we need to provide corpora when instantiating....
      Drone = require('./sentence.drone.js'),
      // harvarddrone = new require('./harvard.drones')({util: util}),
      titlifier_new = new require('../lib/titlifier')({util: util}),
  ...
  let onePoem = function() {
    try {
      let Corpora = require('./corpora.js'),
          ...
          hd = () => new Drone({util: util,
                                sentences: util.pick(corpora.texts.filter((t) => t.name.indexOf('sentence') > -1)).sentences}).generate(),
      ...
      if (strategy === hd) {
        texts = corpora.texts.filter(t => t.name.match(/harvard/ig));
      } else if (config.reduce || util.coinflip(0.3)) {
      ...
      if (!poem.title) {
        do {
          poem.title = titlifier_new.generate(poem.text);
