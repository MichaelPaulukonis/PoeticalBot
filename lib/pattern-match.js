...
let PatternMatcher = function () {
  ...
  const buildLines = (config) => {
    ...
    const matchStrategyFactory = (template) => {
      return function () {
        let n = nlp(text)
        return {
    ...
    if (config.method) {
      ...
      let buildMethods = [`matchStrats`, `posStrats`, `weirdStrats`, `patternStrats`]
    ...
    const lines = (config.matchPattern
    ...
    // er.... we do, don't we?
    return {
      parts: lines.filtered || [],
      description: lines.descr
    }
  }

  const getMatchingLines = function (config) {
    ...
    const name = (typeof config.text === `object`) ? config.text.source : `USER PROVIDED`

    const strategy = buildLines(config)
    const uniques = new Set(strategy.parts)
    const subset = [...uniques]
...
// TODO: figure out how to run this d--n thing over here
